import React from 'react';

import MovieList from './MovieList';

function LibrarySection(props) {
    return (
        <div>
            <div className="d-flex justify-content-start align-items-center">
                <h3 className="text-white ms-2 me-2">{props.title}</h3>
                <button onClick={(e) => { e.preventDefault(); props.onSectionEdit(props.id) }} className="btn btn-danger pbg-accent btn-sm px-2 py-1"><i class="fas fa-edit me-1"></i>Edit</button>
            </div>
            <MovieList movies={props.movies} className="justify-content-lg-start justify-content-center"/>
        </div>
    );
}

export default React.memo(LibrarySection);